...
xhr.onload 이벤트 핸들러 프로퍼티에 바인딩한 이벤트 핸들러는 언제나 <InlineCode>console.log(todos)</InlineCode>가 종료한 이후에 호출된다. 따라서 todos를 출력하는 시점에는 아직 todos 변수에 서버의 응답 결과가 할당되기 이전이다.
...
<InlineCode>.catch(e => console.log(e));</InlineCode> 는
<InlineCode>.then(undefined, e => console.log(e));</InlineCode>와 동일하게
